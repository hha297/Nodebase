...
export const BorderLoadingIndicator = ({ children, className }: { children: ReactNode; className?: string }) => {
      ...
      `}
                                ...
                                <div className={cn('absolute inset-0 overflow-hidden rounded-md', className)}>
                                        <div className="spinner rounded-full bg-[conic-gradient(from_0deg_at_50%_50%,rgb(227,142,107)_0deg,rgb(240,152,107,0)_360deg)]" />
                                </div>
                        </div>
                        {children}
                </>
...
export const NodeStatusIndicator = ({ status, variant = 'border', children, className }: NodeStatusIndicatorProps) => {
        switch (status) {
                ...
                case 'success':
                        return <StatusBorder className={cn('border-green-700', className)}>{children}</StatusBorder>;
                case 'error':
